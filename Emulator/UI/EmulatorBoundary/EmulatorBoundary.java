...
public class EmulatorBoundary extends JFrame {
	...
	public void CreatePollingEntity() {
			...
			protected Void doInBackground() throws Exception {
				...
				while(i == 0) {
					synchronized(Controller.getEmulatorFacade().getSF().getState().getIstance()) {
						while(Controller.getState() == false) {
							...
							Controller.getEmulatorFacade().getSF().getState().getIstance().wait();
						}
						...
						Controller.getEmulatorFacade().getSF().getState().getIstance().notify();
						Thread.sleep(1000);
					}
	...
	public void CreateApplicationLogic() {
			...
			Controller.executeProgram(Configuration.getProgramName(), Configuration.getProgramID(), "C:\\Users\\Stefano\\eclipse-workspace\\NES\\src\\Emulator\\GIOCO.ines");
